...
const ChatInputBase = React.forwardRef<HTMLTextAreaElement, ChatInputProps>(
  ({ className, onSend, onKeyDown, ...props }, ref) => {
    ...
    return (
      <textarea
        className={cn(
          'placeholder:text-official-gray-500 flex max-h-[40vh] min-h-[80px] w-full resize-none overflow-y-auto break-words border-none bg-transparent px-3 py-2 text-base leading-normal focus:outline-hidden focus-visible:outline-hidden focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50',
          className,
        )}
        id="chat-input"
        onKeyDown={(event) => {
