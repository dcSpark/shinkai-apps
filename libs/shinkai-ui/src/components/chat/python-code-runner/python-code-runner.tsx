...
function transformToFileSystemEntry(contents: DirectoryContent[]): FileSystemEntry[] {
  return contents.map((entry) => ({
    name: entry.name,
    type: entry.is_directory ? 'directory' : 'file',
    content: undefined,
    contents: entry.is_directory ? transformToFileSystemEntry(entry.children || []) : undefined,
...
export const PythonCodeRunner = ({ code, jobId, nodeAddress, token }: PythonCodeRunnerProps) => {
  ...
  const [jobContents, setJobContents] = useState<FileSystemEntry[] | null>(null);
  ...
  const { data: fetchedJobContents, refetch: refetchJobContents } = useGetJobContents(
    {
      nodeAddress,
      token,
      jobId,
  ...
  const { pyodide, fileSystemService, initializePyodide } = usePyodideInstance();

  useEffect(() => {
    if (fetchedJobContents) {
      const transformedContents = transformToFileSystemEntry(fetchedJobContents);
        ...
        <Button
          ...
          onClick={async () => {
            try {
              ...
              setIsSyncing(true);
              try {
                await jobService.syncJobFilesToIDBFS(jobContentsResult.data || null);
