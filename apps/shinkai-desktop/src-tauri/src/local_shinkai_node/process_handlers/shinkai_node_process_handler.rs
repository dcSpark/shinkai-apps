...
impl ShinkaiNodeProcessHandler {
    ...
    pub fn default_options() -> ShinkaiNodeOptions {
        ShinkaiNodeOptions {
            ...
            initial_agent_names: Some(if cfg!(windows) { "o_mistral_7b_instruct_q3_K_M" } else { "o_llama3_gradient_8b_instruct_1048k_q3_K_M" }.to_string()),
            initial_agent_models: Some(if cfg!(windows) { "ollama:mistral:7b-instruct-q3_K_M" } else { "ollama:llama3-gradient:8b-instruct-1048k-q3_K_M" }.to_string()),
