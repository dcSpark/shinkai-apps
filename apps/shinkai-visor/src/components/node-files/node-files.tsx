import { Tabs, TabsContent, TabsList, TabsTrigger } from '@shinkai_network/shinkai-ui';
...
export default function NodeFiles() {
  const selectedVectorFsTab = useVectorFsStore((state) => state.selectedVectorFsTab);
  const setSelectedVectorFsTab = useVectorFsStore((state) => state.setSelectedVectorFsTab);
  return (
    <div className="flex h-full flex-col gap-4">
      <Tabs
        className="flex h-full w-full flex-col"
        onValueChange={(value) => setSelectedVectorFsTab(value as 'all' | 'shared-folders')}
      ...
      >
        <TabsList>
          ...
          <TabsTrigger className="flex flex-1 items-center gap-2" value="shared-folders">
            Shared Folders
          </TabsTrigger>
        </TabsList>
